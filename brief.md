...
4. **侦查舰（情报与反制）**  
   - **技能“分析”**：防护值不少于10时，消耗10点防护值秒杀敌方开采舰，并提升友军战列舰攻击力；友方过载恢复防护后可以继续使用。
   - **策略**：优先猎杀敌方开采舰，削弱对手经济。
...
1. **冷却系统**  
   ...
   - **以太密度**：地图格子的数值影响冷却增速（密度越高，冷却增加越少），最低为`0.0001`，移动时尽量选择高密度区域。
...
3. **最终兵器充能**  
   - **每回合策略**：如果所有并列最高出价星球都属于同一队，该队获得1点充能；跨队并列则无人得分。同队多个星球并列不会流局。
