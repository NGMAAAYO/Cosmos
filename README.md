...
每个实体都具有能量值与防护值两个属性。玩家的星球可以通过消耗部分能量点来制造除星球以外的实体。能量点被表示为`R`。新建的战列舰和侦查舰将会有`10`点初始冷却值。
...
~为了防止玩家的代码陷入死循环，或者占用过多能量点导致比赛运行总时间过长，所有实体的运行被限制在20ms之内，超过范围的实体将会被中断运行，并且该回合不会执行任何的动作。~
