...
太长不看：[新手快速入门指南](brief.md)

手把手制作一个自己的AI：[从零开始的 Cosmos 生活](tutorial.md)
...
地图的每一格会有不同的以太密度。以太密度最低为`0.0001`，以太密度越高代表在当前的格子上行动的冷却值越低。地图数据中低于该下限或未填写的格子均按`0.0001`计算。

地图上将会有一些星球作为实体存在。星球不可被摧毁、创造，只能够被易主。在游戏开始时，每方将会有`1`到`3`个初始星球，地图上最多将会有`6`个中立的星球。

公平起见，地图上的一切元素都是对称的。

以太密度为`0.0001`的格子是刻意设计的黑洞隔断。实体可以进入这种格子，但下一次行动会获得至少`10000`点冷却，因而在一场`1000`回合的比赛中等同于失去行动能力。

正式比赛提供六张不依赖随机生成器的确定性双人地图。自然以太由双轴对称的谐波花纹细调；同类型的单星球图和多星球图具有相同的初始全队建造吞吐。

| 类型 | 小型单星球 | 大型多星球 | 战略目标 |
|:--:|:--:|:--:|:--|
| 决斗 | `Fang`（33×33） | `Bastion`（72×48） | 无中立星球，强调直接攻防 |
| 中心 | `Crown`（41×41） | `Crucible`（81×61） | 唯一`75`能量中心星球 |
| 分路 | `Fork`（49×37） | `Delta`（97×65） | 两颗上下镜像的`100`能量星球 |

出生星球的以太会随场景调整：单/多星球决斗图分别为`0.64`和`0.32×2`，中心图为`0.56`和`0.28×2`，分路图为`0.60`和`0.30×2`。由于星球基础冷却为`2`，三组地图的全队初始建造吞吐分别保持为`0.32`、`0.28`和`0.30`次/回合。
...
每一回合，玩家的星球都可以使用一部分能量点来给最终兵器充能。如果所有并列最高出价的星球都属于同一队，该队将产生能量共鸣，其最终兵器累计一个能量点；如果最高出价星球分属不同队伍，则本回合无人得分。
...
- 如果发动者当前防护值不大于`10`，爆发完全无效。否则令发动者当前防护值为`D`，范围内除发动者以外的实体数量为`n`，侦查增益为`K`，每个目标受到的作用量为`P = floor((D - 10) × K / n)`。发动者在过载后被彻底损毁。

  - 作用半径内友方的星球将会增加`P`点能量点。

  - 作用半径内友方的其他实体将会增加`P`点防护值，上限为初始防护值。

  - 作用半径内非友方（敌方或中立）的星球将会损失`P`点能量点，其他实体将会损失`P`点防护值。
...
**开采（被动能力）**：开采舰从被创造后的第`50`回合开始，到第`300`回合为止，为创造它的星球带来能量点收益（如果该星球当回合仍属于本队）。

假设开采舰能量为`E`、舰龄为`a`，令`n(a) = clamp(a - 49, 0, 251)`，截至当前舰龄的累计收益为`M(E,a) = floor(n(a) × E / 300)`。当回合实际收益为`M(E,a) - M(E,a-1)`；失去母星期间错过的收益不会补发。

一艘完整存活至进化的开采舰总共获得`floor(251E / 300)`点收益，始终小于其自身能量，因此矿业无法仅依靠自己的产出形成无限指数增长。
...
**分析（主动能力）**：侦查舰当前防护值必须不少于`10`点。行动会消耗`10`点防护值，入侵行动范围内的一个开采舰并将其摧毁。防护值低于`10`时无法分析；如果友方战列舰的过载后来将其防护值恢复到`10`或以上，它可以继续分析。

通过入侵获得的技术资料将为友方战列舰的能量爆发提供增益。在接下来的`50`回合以内，令所有尚未过期的已分析开采舰能量总和为`S`，友方过载的乘算增益为`K(S) = 2^(min(S, 1000) / 1000)`。`S=100`时增益约为`1.0718`倍，`S=500`时为`sqrt(2)`倍，`S`达到`1000`后封顶为`2`倍。
...
每一回合，所有并列最高充能出价的星球都属于同一队时，该队将会产生能量共鸣，其最终兵器累计一个能量点。`1000` 回合后，拥有更多能量点的最终兵器将会彻底摧毁其余势力的部队，并且赢得比赛。

获得能量共鸣时，所有并列最高出价的星球都会彻底损失充能所消耗的能量点，其余星球只能回收用于充能的能量点的一半。

如果并列最高出价的星球分属不同队伍，则本回合无人获得能量点，所有参与充能的星球都只能回收投入的一半。同队星球并列最高不会导致流局。
...
无线电广播的内容是一个`28`位的非负整数（即`0`-`268435455`）。
